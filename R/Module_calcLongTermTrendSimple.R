...
calcLongTermTrendSimple  <- function(vec.in,gen.in = 4,min.lt.yrs = 20, avg.type = "geomean", tracing=FALSE,
							recent.excl = FALSE){
# this function just does a basic long-term trend calc
...
# vec.in is a vector of values. NA are possible, but will result in NA Trend for any recent time window that includes one or more NA 
# gen.in = use the last gen.in values for the "recent" avg
# avg.type = "mean","geomean", or "median"
# min.lt.yrs = return NA if don't have at least this many years to calculate longtem avg
